...
public class DeviceActivity extends MyActivity implements MooshimeterDelegate {
    ...
    private void refreshTitle() {
        ...
        if (mMeter.getPreference(BLEDeviceBase.mPreferenceKeys.NIMH_BATTERY)) soc_percent*=1.25;//NiMH 1.8 V -> 100%
    ...
    public void onPowerButtonClick(View v) {
        ...
        makePopupMenu(slist, power_button, new NotifyHandler() {
            ...
            public void onReceived(double timestamp_utc, Object payload) {
                ...
                final int listIndex=(Integer)payload;
                runOnUiThread(new Runnable() {
                    @Override
                    public void run() {
                        mathButtonRefresh();
                        switch (listIndex) {
                            case 5://NTC K
                                try {
                                    ...
                                    builder.setTitle("Enter NTC parameters");
                                    builder.setMessage("");
                                    RelativeLayout ntLo=new RelativeLayout(DeviceActivity.this);
                                    RelativeLayout.LayoutParams ntLp=
                                        ...
                                        blp=
                                            new RelativeLayout.LayoutParams(RelativeLayout.LayoutParams
                                                .WRAP_CONTENT,
                                                RelativeLayout.LayoutParams.WRAP_CONTENT);
                                    r0tlp.addRule(RelativeLayout.ALIGN_PARENT_LEFT);
                                    r0lp.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
                                    btlp.addRule(RelativeLayout.ALIGN_PARENT_LEFT);
                                    btlp.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
                                    blp.addRule(RelativeLayout.ALIGN_PARENT_RIGHT);
                                    blp.addRule(RelativeLayout.ALIGN_PARENT_BOTTOM);
                                    final TextView txR0t=new TextView(DeviceActivity.this),txBt=
                                        new TextView(DeviceActivity.this);
                                    final EditText txR0=new EditText(DeviceActivity.this),txB=
                                        new EditText(DeviceActivity.this);
                                    txR0.setSingleLine();
                                    txB.setSingleLine();
                                    txR0t.setText("R0 [kΩ]");
                                    txBt.setText("B [K]");
                                    txR0.setText(String.valueOf(mMeter.gettR0()/1000));
                                    txB.setText(String.valueOf(mMeter.gettB()));
                                    builder.setView(ntLo);
                                    ntLo.setLayoutParams(ntLp);
                                    ntLo.addView(txR0t,r0tlp);
                                    ntLo.addView(txR0,r0lp);
                                    ntLo.addView(txBt,btlp);
                                    ntLo.addView(txB,blp);
                                    ntLo.requestLayout();
                                    builder.setPositiveButton("OK", new DialogInterface.OnClickListener() {
                                        @Override
                                        public void onClick(DialogInterface dialogInterface, int i) {
                                            try {
                                                mMeter.settR0(Float.valueOf(txR0.getText().toString())*1000);
                                                mMeter.settB(Float.valueOf(txB.getText().toString()));
                                            } catch (Exception e) {}
                                        }
                                    });
